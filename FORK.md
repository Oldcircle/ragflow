...
| 2026-04-24 | v0.6 | 同步 `upstream/main` 到 v0.25.0 + 后续 68 commits（merge commit `d146e5fce`）。冲突仅 2 处（`CLAUDE.md` / `testing-result.tsx`），均保留 HEAD；其它 auto-merge。测试 301 pass / 8 skip；前端 build 48.7s OK。上游主要引入：REST API 大重构（user/tenant/stats/plugin/chunk/document/chat 等迁至 `api/apps/restful_apis/`）、DeepSeek v4、docling native chunking、Astraflow provider、title chunk 优化、Go 侧 retrieval_test/think chat/minimax。依赖变动：`langfuse` 2.60.0 → 4.5.0，新增 `debugpy>=1.8.13`。|
| 2026-04-24 | v0.7 | Phase 2.6 v0.7 + v0.8：web_search + web_fetch（Tavily / httpx + BS4 + SSRF 三段防御 + redirect 白名单 + markdownify 保结构）；tool-availability section + 负面枚举止住 DeepSeek"你有啥工具"幻觉；template → tool_names 流通让研究模板真带 web；15 题黄金集加 Q11-Q15 域外防幻觉（纽约/2030/虚构文件号/跨话题/伪前提）；Q13 的 runner 死循环硬 guard（prompt 2 次空 + runner 3 次空兜底）；LLM-as-judge 评分器。手工校核 15 题 4.93/5，域外防幻觉 5.00 首次过 4.5 商用底线。+47 测试（376 pass）|
| 2026-04-25 | v0.10 | **Phase 2.7 完整落地**（附件协议 + 下载-验证-归档）。深扒 `claude-code-ref/src/utils/attachments.ts` 后决定对齐两条：attachment-as-independent-message pattern（独立 DB 表 `agent_v2_attachment`）+ 复用 v0.4 `submit_plan + plan_gate` 作 staging（ref 全库无 PendingQueue）。5 stage 闭环：①DB + HTTP + ctx 注入 ②2 新工具 `doc_archive_attachment` + `web_fetch_to_attachment`（前者 plan_gated，后者 openWorld，共用 50 MB cap + xxh128 dedup）③`submit_plan.preview` 扩展（8KB markdown_excerpt + 前端折叠卡片）④Composer 文件选择器 + drag-drop + AttachmentChip + useAttachments hook ⑤两真机 smoke + G13 人工用例。MIME 白名单含图片（走 `FileType.VISUAL`，OCR 下游走 RAGFlow 原生 PaddleOCR/MinerU 或 Stage 2+ 扩展）。**工具总数 20 → 22；测试 329 → 452 pass / 8 skip**（+76 新用例覆盖 attachments + doc_archive_attachment + web_fetch_to_attachment + submit_plan_preview）。MinIO bucket `agent-v2-attachments`（S3-compliant naming）|
