...
ARCHIVIST_TOOL_RULES = [
    ...
    "the target KB. For images, the tool synchronously runs PaddleOCR and "
    "indexes a markdown sub-document (`<name>.ocr.md`) instead of the raw "
    "image; the response carries `ocr_signal` (rich / low / none). Surface "
    "that signal to the user in your step-done line — for `none` images, "
    "tell them the markdown is just a stub and the image will need a "
    "vision-LLM pass. Do NOT fabricate attachment_id — take it from "
